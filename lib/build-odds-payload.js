...
const NCAAF_FCS_CACHE_KEY = 'odds_feed:americanfootball_ncaaf_fcs';
const ODDS_API_USAGE_KEY = 'odds_api_usage_history';
const NCAAF_FCS_CACHE_TTL_SECONDS = 15 * 60;
const NCAAF_FCS_ACTIVE_REFRESH_MS = 2 * 60 * 1000;
const NCAAF_FCS_NORMAL_REFRESH_MS = 15 * 60 * 1000;

function isFcsFeedActive(games) {
...
function normalizeTeamName(name) {
  ...
  const aliases = {
    ...
    citadelbulldogs: 'thecitadelbulldogs',
    umassminutemen: 'massachusettsminutemen',
    albany: 'ualbanygreatdanes',
    liusharks: 'longislanduniversitysharks',
    ...
    nichollsstatecolonels: 'nichollscolonels',
...
for (const oddsApiSport of oddsApiSports) {
  if (oddsApiSport === 'americanfootball_ncaaf_fcs') {
    const cachedFcs = await redis.get(NCAAF_FCS_CACHE_KEY);

    if (
      cachedFcs &&
      Array.isArray(cachedFcs.games) &&
      Number.isFinite(cachedFcs.fetchedAt)
    ) {
      const refreshInterval = isFcsFeedActive(cachedFcs.games)
        ? NCAAF_FCS_ACTIVE_REFRESH_MS
        : NCAAF_FCS_NORMAL_REFRESH_MS;

      const cacheAge = Date.now() - cachedFcs.fetchedAt;

      if (cacheAge < refreshInterval) {
        oddsData.push(...cachedFcs.games);
        continue;
      }
    }
  }
  ...
  console.log('ODDS API USAGE:', oddsApiUsage);

  await redis.lpush(
    ODDS_API_USAGE_KEY,
    JSON.stringify(oddsApiUsage)
  );
  ...
  if (Array.isArray(sportData)) {
    ...
    if (oddsApiSport === 'americanfootball_ncaaf_fcs') {
      await redis.set(
        ...
        {
          ex: NCAAF_FCS_CACHE_TTL_SECONDS
        }
      );
    }
  }
    ...
    const freshGames = Array.from(
      new Map(
        gamesData.map(game => {
          ...
          const gameKey =
            normalizedGame?.espnStatus?.id
              ? `espn_${normalizedGame.espnStatus.id}`
              : makeGameKey(
                  normalizedGame.away,
                  normalizedGame.home,
                  normalizedGame.commence_time
                );

          return [gameKey, normalizedGame];
        })
      ).values()
    );
